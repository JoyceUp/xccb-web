...
<div class="wrapper">
    <div class="container-fluid myTables">
        <div class="page-title-box">
            <div class="btn-group ">
                <ol class="breadcrumb hide-phone p-0 m-0">
                    ...
                    <li class="breadcrumb-item active">分润报表</li>
                </ol>
            </div>
        </div>
        <div id="main" class="card-box ddbb">
            ...
            <p class="pull-right">
                <span>总手数：{{top.volume}}</span>
                <span class="ml-3">总获利：{{top.profit}}</span>
                <span class="ml-3">总分润：{{top.amount}}</span>
                <span class="ml-3">可提分润：{{top.withdrawamount}}</span>
            </p>
            <table class="table table-striped table-hover table-sm frbbtab myhidden">
            ...
            <div class="comTable myhBox">
                <div class="b_flex b_between w-10 th-head">
                    ...
                    <p class="text-right">分润</p>
                </div>
                <ul class="b_flex b_column">
                    <li class="b_flex b_wrap box-s" v-for="(record,index) in records" :key="record.id"
...
export default {
    ...
    methods: {
        ...
        load(page) {
            ...
            http.get(api + 'Rebate/getlist', {params})
                .then(function (response) {
                    const data = response.data;
                    if ('0' === data.code) {
                        ...
                        vm.top.amount = data.data.amount / 100.0
                        vm.top.profit = data.data.profit / 100.0
                        vm.top.withdrawamount = data.data.withdrawamount / 100.0
                        vm.top.volume = data.data.volume
                    }
                })
                .catch(function (error) {
                    console.log(error)
